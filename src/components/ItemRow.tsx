...
const ItemRow: FC<{ entires: IAogEntry[] | undefined; currency: string }> = ({
  entires,
  currency,
}) => {
    ...
    <div>
      <Space size="large">
        ...
        <div className={styles.viewPort}>
          <Space
            size="large"
            style={{ minWidth: 160 * (t3Item?.stages.length || 0) }}
          >
            {t3Item?.stages.map((s) => (
        ...
        {t2Item && (
          <div className={`${styles.t2ScollView} ${styles.t2Display}`}>
            <Space
              size="large"
              style={{ minWidth: 160 * (t2Item?.stages.length || 0) }}
            >
              {t2Item?.stages.map((s) => (
                <StageCell stage={s} key={s.stage} />
              ))}
